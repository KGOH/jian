...
(defn page []
  [:article.content
   [:section
    ...
    [:p.l "Доступ к дополнительным клавишам происходит через дополнительные слои " [:img.icon {:src "images/down-icon.svg"}] " ("[:i "fn1"]") и " [:img.icon {:src "images/up-icon.svg"}] " ("[:i "fn2"]"). Сейчас я расскажу, в чем особенность раскладки, слоев, и как ими пользоваться."]]
   [:div
    ...
    [:div.flex
     [:div
      ...
      [:figure
       [:img.fullwidth {:src "images/planck.svg"}]
...
на обычной клавиатуре."]
     ...
     [:aside
      ...
      [:div.to-bottom
       [:p.l. [:img {:src "images/legend-tip.svg"}] [:i "У некоторых клавиш двойное действие. Их можно нажать, либо удерживать"]]]]
     ]
    [:h3 "Раскладка ЙЦУКЕН"]
    [:figure.ycuken
     [:img {:src "images/ycuken.svg"}]]
    [:h3 "Раскладка QWERTY"]
    [:figure.qwerty
     [:img.fullwidth {:src "images/qwerty.svg"}]]
    ]
   [:div
    ...
    [:div.flex
     [:div
      ...
      [:p "Кнопки активации слоев есть на каждой половинке Jian
и распложены симметрично. Это позволяет одной рукой нажать любую клавишу на верхнем или нижнем слое. На той же клавиатуре Plank приходится задействовать обе руки."]
      ]
     [:aside]]
    [:figure.layers
     [:img {:src "images/layers.svg"}]]]
   [:div
    [:h2 "Нижний слой — " [:img.icon {:src "images/down-icon.svg"}]]
    [:div.flex
     [:div
    ...
    [:h3 "Раскладка ЙЦУКЕН"]
    [:figure.ycuken
     [:img {:src "images/down-layer-ycuken.svg"}]]
    [:h3 "Раскладка QWERTY"]
    [:figure.qwerty
     [:img.fullwidth {:src "images/down-layer-qwerty.svg"}]]
    ]
   [:div
    [:h2 "Верхний слой — " [:img.icon {:src "images/up-icon.svg"}]]
    [:div.flex
     [:div
     ...
     [:aside]]
    [:figure.layers
     [:img {:src "images/up-layer.svg"}]]]
   [:div
    [:h2 "Слой настроек"]
    [:div.flex
     [:div
      [:p "Слой настроек вызывается при одновременном удерживании клавиш " [:img.icon {:src "images/down-icon.svg"}] " и " [:img.icon {:src "images/up-icon.svg"}] ". Все настройки расположены зеркально на обеих половинках Jian, чтобы ими можно было пользоваться даже тогда, когда подключена лишь одна часть."]]
     [:aside]]
    [:figure.layers
     [:img {:src "images/settings-layer.svg"}]]]
