...
(defn page []
  [:article {:class (cu/c "content")}
   [:section
    [:div
     [:header
      [:h1 "Клавиатура Jian и её раскладка"]
      [:p {:class (cu/c "l")} "Jian — сплит-клавиатура на 44 клавиши. Несмотря на свой размер, она содержит все 105 кнопок полноразмерной клавиатуры. Нажимать их можно не перемещая руки с основного ряда."]]
     [:p {:class (cu/c "l")} "Доступ к дополнительным клавишам происходит через дополнительные слои "
   ...
   [:section
    [:div
     ...
     [:div {:class (cu/c "flex")}
      ...
      [:aside
       [:small {:class (cu/c "facts-header")} "Что такое Planck?"]
       [:ul {:class (cu/c "ok-before")}
    ...
    [:div
     ...
     [:div {:class (cu/c "flex")}
      [:div
       ...
       [:p "Все клавиши нижнего ряда имеют двойное действие. Их можно нажимать или удерживать. Действия при удержании клавиш отмечены на раскладке "[:span {:class (cu/c "blue")} "синим цветом"]"."]
...
на обычной клавиатуре."]]
      [:aside
       [:p {:class (cu/c "l")} [:i "На клавиатуре Jian помещаются все буквы раскладки ЙЦУКЕН, включая Ё, Х и Ъ"]]
       [:div {:class (cu/c "to-bottom")}
        [:p {:class (cu/c "l")} [:img (res-src "images/legend-tip.svg")] [:br] [:i "У некоторых клавиш двойное действие. Их можно нажать, либо удерживать"]]]]]
     [:h3 "Раскладка ЙЦУКЕН"]
     [:figure {:class (cu/c "ycuken")}
      [:img (res-src "images/ycuken.svg")]]
     [:h3 "Раскладка QWERTY"]
     [:figure {:class (cu/c "qwerty")}
   ...
   [:section
    [:div
     ...
     [:div {:class (cu/c "flex")}
      [:div
       [:p "Дополнительные клавиши Jian находятся на «верхнем» и «нижнем» слоях. Чтобы попасть на нижний слой, нужно зажать кнопку "[:b {:class (cu/c "green")} [:i "fn1"]]". Чтобы попасть на верхний — зажать "[:b {:class (cu/c "orange")} [:i "fn2"]]"."]
    ...
    [:div
     ...
     [:figure {:class (cu/c "qwerty")}
      [:img (merge (res-src "images/down-layer-qwerty.svg")
                   {:class (cu/c "fullwidth")})]]
     [:div {:class (cu/c "info-table")}
      [:div
       [:h4 "Сравнение ввода некоторых символов на обычной и Jian-раскладке"]
       [:div {:class (cu/c "table")}
        [:div {:class (cu/c "thead")}
         [:div {:class (cu/c "th")} "Символ"]
         [:div {:class (cu/c "th")} "Об. раскладка"]
         [:div {:class (cu/c "th")} "Раскладка Jian"]]
        [:div {:class (cu/c "tr")}
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-1_a.svg")]]
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-1_b.svg")]]
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-1_c.svg")]]
         ]
        [:div {:class (cu/c "tr")}
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-2_a.svg")]]
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-2_b.svg")]]
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-2_c.svg")]]]
        [:div {:class (cu/c "tr")}
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-3_a.svg")]]
         [:div {:style {:textAlign "right"}
                :class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-3_b.svg")]]
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/1/Jian-key-T1-3_c.svg")]]]]]
      [:aside [:p {:class (cu/c "l")} [:i "Как правило, чтобы ввести дополнительные символ на Jian понадобится столько же действий, сколько и на обычной клавиатуре."]]]]]]
   [:section
    [:div
     ...
     [:div {:class (cu/c "info-table")}
      [:div
       ...
       [:div {:class (cu/c "table")}
        [:div {:class (cu/c "thead")}
         [:div {:class (cu/c "th")} "Обычная раскладка"]
         [:div {:class (cu/c "th")} "Раскладка Jian"]]
        [:div {:class (cu/c "tr")}
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/2/Jian-key-T2-1_a.svg")]]
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/2/Jian-key-T2-1_b.svg")]]]
        [:div {:class (cu/c "tr")}
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/2/Jian-key-T2-2_a.svg")]]
         [:div {:class (cu/c "td")} [:img (res-src "images/tables/2/Jian-key-T2-2_b.svg")]]]
        [:div {:class (cu/c "tr")}
         [:div {:style {:textAlign "right"}
                :class (cu/c "td")} [:img (res-src "images/tables/2/Jian-key-T2-3_a.svg")]]
         [:div {:style {:textAlign "right"}
                :class (cu/c "td")} [:img (res-src "images/tables/2/Jian-key-T2-3_b.svg")]]]]]
      [:aside [:p {:class (cu/c "l")} [:i "Раскладка Jian продумана и покрывает все 105-клавиш полноразмерной клавиатуры."]] [:p {:class (cu/c "l")} [:i "Их можно легко переназначить при помощи онлайн-конфигуратора для прошивки QMK."]]]]]
    [:section
     [:div
      [:h2 "Слой настроек"]
      [:div {:class (cu/c "flex")}
       [:div
        [:p "Слой настроек вызывается при одновременном удерживании клавиш " [:img  (merge (res-src "images/down-icon.svg") {:class (cu/c "icon")})] " и " [:img  (merge (res-src "images/up-icon.svg") {:class (cu/c "icon")})] ". Все настройки расположены зеркально на обеих половинках Jian, чтобы ими можно было пользоваться даже тогда, когда подключена лишь одна часть."]]
       [:aside]]
      [:figure {:class (cu/c "layers")}
      ...
      [:div {:class (cu/c "info-table")}
       ...
       [:aside
        [:p {:class (cu/c "l")} [:i "Если вы решите сделать собственный вариант раскладки, то её активацию удобно расположить на слое «Настроек». Например, можно сделать специальную раскладку для игр, или для работы с планшетом."]]
        [:table {:class (cu/c "table")}
         [:tbody
          [:tr
    ...
    [:div
     ...
     [:div {:class (cu/c "flex")}
      [:div
       ...
       [:p "👉 Скачать версию для печати — " [:a (cu/a {:href (resource-path "cheat_sheet.pdf")}) "pdf"]]]
      [:aside]]
     [:h3 "Раскладка ЙЦУКЕН"]
     [:figure {:class (cu/c "layers")}
   ...
   [:section
    [:div
     ...
     [:p {:class (cu/c "l")} "В базовый комплект Jian входит: печатная плата, корпус, два контроллера Pro Micro и электронные компоненты (диоды, разъёмы и т. п.). Вам нужно отдельно приобрести USB Micro, TRRS-TRRS–кабеля, свитчи и кейкапы. Светодиоды и ленту подсветки — опционально. А затем самостоятельно набор собрать."]
     [:p {:class (cu/c "l")} "💰 Цена базового комплекта — 6000 руб."]
     [:p {:class (cu/c "l")} "Цена полного комплекта со сборкой обговаривается индивидуально и зависит от стоимости свичей, кейкапов и дополнительных опций."]
     [:p {:class (cu/c "l")} "Заказать клавиатуру можно записавшись в групповую покупку, а получить консультацию — через Telegram."]
     [:p {:class (cu/c "l")} "💰 Купить Jian — "[:a {:href "https://killswit.ch/product/jian/"} "https://killswit.ch/product/jian/"]
   ...
   [:footer [:div {:class (cu/c "to-bottom")} "Редактор — Влад Гороховский, " [:a {:href "https://t.me/gorohovsky"} "https://t.me/gorohovsky"]]]])
